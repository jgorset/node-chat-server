exports.Client = function(socket) {
  ...
  return {
    ...
    commands: {
      ...
      // Promote another client to administrator.
      promote: function(nick) {
        if(this.administrator) {
          Chat.send(this.nick + " granted " + nick + " administrative privileges.\n", "! ");

          Chat.clients.forEach(function(client) {
            if(client.nick == nick) client.administrator = true;
          });
        } else {
          this.send("You must be an administrator to issue this command.\n", "! ");
        }
